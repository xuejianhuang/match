...
public class RoleService extends BaseDao<Role> implements IRoleService
{
	...
	public Tree getPrivilegesTree(Operator oper, String roleId)
	{
		...
		else
		{
			obj=new Object[1];
			obj[0]=roleId;
		   list2 = getHibernateTemplate().find("from Menu where id in(select menuId from RoleMenu where roleId=?) order by order",obj);// 查出对应角色的目录结构
			...
			String[] menuOrderList)
	{
		...
		{
			obj[0]=roleId;
			rolePrivilege = getHibernateTemplate().find("from Menu where id in(select menuId from RoleMenu where roleId=?) order by order",obj);// 查出对应角色的目录结构
	...
	private List<RoleMenu> findRemove(List<Menu> roleList, String[] menuIdList,String roleId)
	{
		...
		{
			obj[1]=menu.getId();
		 roleMenu=(RoleMenu) getHibernateTemplate().find("from RoleMenu where roleId=? and menuId=?",obj).get(0);
		 remove.add(roleMenu);
		}
		return remove;
	}
